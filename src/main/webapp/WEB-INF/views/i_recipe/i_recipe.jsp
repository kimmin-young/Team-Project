...
<title>i_recipe/i_recipe.jsp</title>
<style type="text/css">
#contentTbl_1 {
	width: 80%;
...
<body>
	...
	<div align="left" style="width: 690px;">
		<b> <a
			...
			href="i_recipe.go?shape=gallery&sort=r_no">개인레시피로</a>
		</b>
	</div>
			...
			<td align="right"><c:choose>
					<c:when test="${sort == 'r_no'}">
						<form method="get" style="margin-bottom: 1px;">
							<input type="hidden" name="p" value="${curPage }"> <input
								type="hidden" name="shape" value="${shape }">
		...
		<c:when test="${shape == 'list'}">
			...
			<table id="contentTbl_1" align="center" border="1">
				...
				<c:forEach var="rc" items="${rcps }">
					<tr>
						<td align="left" width="6%" style="color: orange;">${rc.r_no }</td>
						<c:choose>
							<c:when test="${rc.r_photo == 'defaultThumbnail.png'}">
								<td width="7%" align="center"><a
									href="i_recipe.rcp.detail?p=${curPage }&shape=${shape }&sort=${sort }&r_no=${rc.r_no }${f }${s }"><img
										class="PRimg" src="resources/img/s_defaultThumbnail.png"></a></td>
							</c:when>
							<c:when test="${rc.r_example == 'O' }">
								<td width="7%" align="center"><a
									href="i_recipe.rcp.detail?p=${curPage }&shape=${shape }&sort=${sort }&r_no=${rc.r_no }${f }${s }"><img
										class="PRimg" src="${rc.r_photo }"></a></td>
							</c:when>
							<c:otherwise>
								<td width="7%" align="center"><a
									href="i_recipe.rcp.detail?p=${curPage }&shape=${shape }&sort=${sort }&r_no=${rc.r_no }${f }${s }"><img
										class="PRimg" src="resources/img/s_${rc.r_photo }"></a></td>
							</c:otherwise>
						</c:choose>
						<td align="center" width="10%">${rc.r_category }</td>
						<td align="left" width="30%" style="font: bold"><a
							href="i_recipe.rcp.detail?p=${curPage }&shape=${shape }&sort=${sort }&r_no=${rc.r_no }${f }${s }"><b
								style="font-size: 12pt;">${rc.r_title }</b></a> <c:if
								test="${rc.r_ended == 'O' }">
							...
							<c:when test="${rc.r_ended == 'O' }">
								<c:choose>
									<c:when test="${rc.r_star_avg == null}">
										<td align="left" width="15%">별점 0.00</td>
									</c:when>
									<c:otherwise>
										<td align="left" width="15%" style="color: orange;"><c:set
						...
						<td align="right" width="10%"
							style="color: gray; text-align: center;">${rc.r_writer }</td>
						<td align="right" align="center"
							style="color: gray; text-align: center;"><fmt:formatDate
								value="${rc.r_date }" type="date" /></td>
						<td align="right" width="8%"
							style="color: gray; text-align: center;">${rc.r_seen }</td>
								...
								<table>
									<tr>
										<c:choose>
											<c:when test="${rc.r_photo == 'defaultThumbnail.png'}">
												<td><a
													href="i_recipe.rcp.detail?p=${curPage }&shape=${shape }&sort=${sort }&r_no=${rc.r_no }${f }${s }">
														<img class="Rimg"
									...
									<tr>
										<td id="recipeTitle"><a
											href="i_recipe.rcp.detail?p=${curPage }&shape=${shape }&sort=${sort }&r_no=${rc.r_no }${f }${s }"><b>${rc.r_title }</b></a>
										</td>
									</tr>
									<tr>
										<td class="listSubText" style="color: orange;">별점 <c:set
												var="star" value="${rc.r_star_avg }" /> <c:choose>
												<c:when test="${star == 5 }">
							&#9733;&#9733;&#9733;&#9733;&#9733;(${rc.r_star_avg })
						</c:when>
		...
		<table align="center">
			<tr align="center">
				...
				<td><input type="text" name="search" maxlength="20"
					...
					value="${sort }"> <c:if test="${!empty param.category }">
						<input type="hidden" name="category" value="${param.category }">
					</c:if></td>
				<td>
					<button>검색</button>
	...
	<ul style="text-align: center;">
		...
		<!-- 첫 페이지 -->
		<li style="display: inline-block; cursor: pointer;"><a
			href="i_recipe.page?p=1&shape=${shape }&sort=${sort }${f }${s }">◀◀</a></li>
		<!-- 이전 페이지 -->
		<c:if test="${startNum != 1 }">
			<li style="display: inline-block; cursor: pointer;"><a
				href="i_recipe.page?p=${startNum-10 }&shape=${shape }&sort=${sort }${f }${s }">이전</a></li>
		</c:if>
					...
					<c:when test="${allPageCount>0 }">
						<c:forEach var="i" begin="0" end="${(allPageCount%10)-1 }">
							<li
								...
								<a
								href="i_recipe.page?p=${startNum+i }&shape=${shape }&sort=${sort }${f }${s }">${startNum+i }</a>
							</li>
						</c:forEach>
					</c:when>
					<c:otherwise>
						<c:forEach var="i" items="0">
							<li
								style="cursor: pointer; display: inline-block; color: ${(curPage==startNum+i)?'#FF5E00':'black'};">
								<a
								href="i_recipe.page?p=${startNum+i }&shape=${shape }&sort=${sort }${f }${s }">${startNum+i }</a>
							</li>
						</c:forEach>
		...
		<c:if test="${startNum+9 < allPageCount }">
			<li style="display: inline-block; cursor: pointer;"><a
				href="i_recipe.page?p=${startNum+10 }&shape=${shape }&sort=${sort }${f }${s }">다음</a></li>
		</c:if>

		<!-- 마지막 페이지 -->
		<li style="display: inline-block; cursor: pointer;"><a
			href="i_recipe.page?p=${allPageCount }&shape=${shape }&sort=${sort }${f }${s }">▶▶</a></li>

	</ul>

	<div style="position: absolute; right: 35px; bottom: 105px;">
		<button onclick="goWrite();">레시피 작성</button>
	</div>
