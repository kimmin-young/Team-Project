...
<body>
	...
	<table align="center" id="btnTbl">
		<tr>
			<td align="left"><button onclick="goWrite();">레시피 작성</button></td>
					...
					<tr>
						...
						<c:choose>
							<c:when test="${rc.r_photo == 'defaultThumbnail.png'}">
								<td width="7%" align="center"><img width="50px" height="50px"
									src="resources/img/s_defaultThumbnail.png"></td>
							</c:when>
							...
							<c:otherwise>
								<td width="7%" align="center"><img width="50px" height="50px"
									src="resources/img/s_${rc.r_photo }"></td>
							</c:otherwise>
						</c:choose>
						<td align="left" width="10%">[ ${rc.r_category } ]</td>
						<td align="left" width="40%" style="font: bold"><a
							href="i_recipe.rcp.detail?p=${curPage }&shape=${shape }&sort=${sort }&r_no=${rc.r_no }&from=my${s }"><b>${rc.r_title }</b></a>
							...
							<td>
								<table>
									<tr>
										<c:choose>
											<c:when test="${rc.r_photo == 'defaultThumbnail.png'}">
												<td><a href="i_recipe.rcp.detail?p=${curPage }&shape=${shape }&sort=${sort }&r_no=${rc.r_no }&from=my${s }">
												<img class="Rimg" src="resources/img/s_defaultThumbnail.png"></a></td>
											</c:when>
											<c:when test="${rc.r_example == 'O' }">
												<td><a href="i_recipe.rcp.detail?p=${curPage }&shape=${shape }&sort=${sort }&r_no=${rc.r_no }&from=my${s }">
												<img width="100px" height="100px" class="Rimg" src="${rc.r_photo }"></a></td>
											</c:when>
											<c:otherwise>
												<td><a href="i_recipe.rcp.detail?p=${curPage }&shape=${shape }&sort=${sort }&r_no=${rc.r_no }&from=my${s }">
												<img class="Rimg" src="resources/img/s_${rc.r_photo }"></a></td>
											</c:otherwise>
										</c:choose>
									...
									<tr>
										<td id="recipeTitle"><a
											href="i_recipe.rcp.detail?p=${curPage }&shape=${shape }&sort=${sort }&r_no=${rc.r_no }&from=my${s }">
												<b>${rc.r_title }</b>
										</a></td>
									</tr>
									<tr>
	...
	<ul style="text-align: center;">
		...
		<li style="display: inline-block;">
			<a href="i_recipe.page?p=1&shape=${shape }&sort=${sort }${f }${s }">◀◀</a></li>
		<!-- 이전 페이지 -->
		<c:if test="${startNum != 1 }">
			<li style="display: inline-block;">
				<a href="i_recipe.page?p=${startNum-10 }&shape=${shape }&sort=${sort }${f }${s }">이전</a></li>
		</c:if>
				...
				<c:choose>
					<c:when test="${allPageCount>0 }">
						<c:forEach var="i" begin="0" end="${(allPageCount%10)-1 }">
							<li style="display: inline-block; color: ${(curPage==startNum+i)?'#FF5E00':'black'};">
								<a href="i_recipe.page?p=${startNum+i }&shape=${shape }&sort=${sort }${f }${s }">${startNum+i }</a>
							</li>
						</c:forEach>
					</c:when>
					<c:otherwise>
		...
		<c:if test="${startNum+9 < allPageCount }">
			<li style="display: inline-block;">
				<a href="i_recipe.page?p=${startNum+10 }&shape=${shape }&sort=${sort }${f }${s }">다음</a></li>
		</c:if>

		<!-- 마지막 페이지 -->
		<li style="display: inline-block;">
			<a href="i_recipe.page?p=${allPageCount }&shape=${shape }&sort=${sort }${f }${s }">▶▶</a></li>

	</ul>

	<!-- 검색 -->
	<form action="i_recipe.search.my" method="get">
		<table align="center">
			<tr align="center">
				<c:choose>
